...
while(currentPrint ==1 || ant.location(2) >= 0)
    ...
    %drawnow;
   % plotshit(nestLocation,foodSourceLocation,nodeLocation,ant,ground,currentPrint,dt,printFlag);
...
function plotshit(nestLocation, foodSourceLocation, nodeLocation,ant,ground,currentPrint,dt,printFlag)
     cla;
    hold on;
    minv = min([nestLocation foodSourceLocation nodeLocation]');
    maxv = max([nestLocation foodSourceLocation nodeLocation]');
